				       ...
				       struct svfs_ops *svfs_ops)
{
  ...
  // check is iso9660/udf format
  if (DVDUDFVolumeInfo(dvd, volid, volid_size, volsetid, volsetid_size) == 0) {
	  dvd->isImageFile = 1;
  }
  else if (DVDISOVolumeInfo(dvd, volid, volid_size, volsetid, volsetid_size) == 0) {
	  dvd->isImageFile = 2;
  }
  else {
	  dvdinput_close(dev);
...
#if !defined(WIN32) && !defined(WII) && !defined(PS3) /* don't have fchdir, and getcwd( NULL, ... ) is strange */
